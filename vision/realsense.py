...
class realsense():
    ...
    def get_rs_frames(self):
        ...
        filtered = self.spat_filter.process(aligned_depth_frame)
        filtered = self.temp_filter.process(filtered)

        return (color_frame, filtered)

    def cord_3d(self, depth_frame, pixel_2d):
        depth_intrin = self.profile.as_video_stream_profile().intrinsics
        distance = depth_frame.get_distance(pixel_2d[0], pixel_2d[1])
